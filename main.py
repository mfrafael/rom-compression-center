...
class RomCompressionGUI(QWidget):
    ...
    def __init__(self):
        ...
        self.status_update_signal.connect(self.set_status_label)
    ...
    def toggle_queue(self):
        if not self.queue_running:
            ...
            self.queue_worker = QueueWorker(queue, self.queue_table, self.compress_checkbox.isChecked(), platform_handlers, verbose=self.verbose_checkbox.isChecked())
            self.queue_worker.moveToThread(self.queue_thread)
            self.queue_worker.status_update.connect(self.set_status_label)
    ...
    def apply_table_search(self):
        ...
        for table in [self.table_uncompressed, self.table_compressed]:
            for row in range(table.rowCount()):
                match = True if filters else False
                for f in filters:
                    found = False
                    ...
                    if not found:
                        match = False
                        break
                table.setRowHidden(row, not match if filters else False)
        ...
        for table in [self.table_uncompressed, self.table_compressed]:
            for row in range(table.rowCount()):
                ...
                if item and item.text() == file_path:
                    ...
                    # Você pode atualizar outras colunas se quiser
                    break
